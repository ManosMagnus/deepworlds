...
def run(supervisorEnv):
    ...
    while not solved and episodeCount < episodeLimit:
        ...
        for step in range(supervisorEnv.stepsPerEpisode):
            ...
            for i in range(2):
                trans = Transition(state[i], selectedActions[i],
                                   actionProbs[i][-1], reward, newState[i])
        ...
        avgActionProb = [round(mean(actionProbs[i]), 4) for i in range(2)]
